...
export function degreesLong(radians) {
  let degrees = (radians / constants.pi * 180) % (360);
  if (degrees > 180) {
    degrees = 360 - degrees;
  } else if (degrees < -180) {
    degrees = 360 + degrees;
...
export function dopplerFactor(myLocation, position, velocity) {
  const currentRange = Math.sqrt(Math.pow(position.x - myLocation.x, 2) +
  ...
  const nextRange = Math.sqrt(Math.pow(nextPos.x - myLocation.x, 2) +
...
export function gstimeFromDate(date) {
  return gstime(jday(date));
}

export function dpper(satrec, dpperParameters) {
  /* -----------------------------------------------------------------------------
     *
     *                           procedure dpper
     *
  ...
  const e3 = satrec.e3;
  const ee2 = satrec.ee2;
  const peo = satrec.peo;
  const pgho = satrec.pgho;
  const pho = satrec.pho;
  const pinco = satrec.pinco;
  const plo = satrec.plo;
  const se2 = satrec.se2;
  const se3 = satrec.se3;
  const sgh2 = satrec.sgh2;
  const sgh3 = satrec.sgh3;
  const sgh4 = satrec.sgh4;
  const sh2 = satrec.sh2;
  const sh3 = satrec.sh3;
  const si2 = satrec.si2;
  const si3 = satrec.si3;
  const sl2 = satrec.sl2;
  const sl3 = satrec.sl3;
  const sl4 = satrec.sl4;
  const t = satrec.t;
  const xgh2 = satrec.xgh2;
  const xgh3 = satrec.xgh3;
  const xgh4 = satrec.xgh4;
  const xh2 = satrec.xh2;
  const xh3 = satrec.xh3;
  const xi2 = satrec.xi2;
  const xi3 = satrec.xi3;
  const xl2 = satrec.xl2;
  const xl3 = satrec.xl3;
  const xl4 = satrec.xl4;
  const zmol = satrec.zmol;
  const zmos = satrec.zmos;
...
export function dspace(dspaceParameters) {
  /*-----------------------------------------------------------------------------
     *
     *                           procedure dspace
     *
  ...
  let irez = dspaceParameters.irez,
...
export function sgp4(satrec, tsince) {
  ...
  const vkmpersec = constants.earthRadius * constants.xke / 60.0;
  ...
  const mvt = rdotl - nm * temp1 * satrec.x1mth2 * sin2u / constants.xke;
...
export function dscom(dscomParameters) {
  /*-----------------------------------------------------------------------------
     *
     *                           procedure dscom
     *
  ...
  let epoch = dscomParameters.epoch,
...
export function dsinit(dsinitParameters) {
  /*-----------------------------------------------------------------------------
     *
     *                           procedure dsinit
     *
  ...
  let cosim = dsinitParameters.cosim,
...
export function initl(initlParameters) {
  /*-----------------------------------------------------------------------------
     *
     *                           procedure initl
     *
  ...
  const ao = Math.pow(constants.xke / no, constants.x2o3);
...
export function sgp4init(satrec, sgp4initParameters) {
  /*-----------------------------------------------------------------------------
     *
     *                             procedure sgp4init
     *
  ...
  let opsmode = sgp4initParameters.opsmode,
  ...
  const ss = 78.0 / constants.earthRadius + 1.0;
  //  sgp4fix use multiply for speed instead of pow
  const qzms2ttemp = (120.0 - 78.0) / constants.earthRadius;
  ...
  if (omeosq >= 0.0 || satrec.no >= 0.0) {
    ...
    if (perige < 156.0) {
      ...
      //  sgp4fix use multiply for speed instead of pow
      const qzms24temp = (120.0 - sfour) / constants.earthRadius;
    ...
    if (2 * constants.pi / satrec.no >= 225.0) {
      ...
      const dsinitResult = dsinit(dsinitParameters);
...
export function twoline2satrec(longstr1, longstr2) {
  ...
  const opsmode = 'i';
  const xpdotp = 1440.0 / (2.0 * constants.pi); //  229.1831180523293;
  let revnum = 0;
  let elnum = 0;
  let year = 0;
  ...
  satrec.bstar = parseFloat(`${longstr1.substring(53, 54)
  ...
  satrec.inclo *= constants.deg2rad;
  satrec.nodeo *= constants.deg2rad;
  satrec.argpo *= constants.deg2rad;
  satrec.mo *= constants.deg2rad;
